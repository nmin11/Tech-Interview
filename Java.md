...
그리고 오버로딩은 같은 클래스 안에서 동일한 메소드 이름을 사용했을 때에도, 매개 변수의 갯수나 타입에 따라 메소드를 달리 사용할 수 있도록 해주는 기능입니다.  
단, 리턴 타입만 다르다면 오버로딩을 할 수 없습니다.  
보통의 경우 함수 한 개가 한 가지의 기능을 담당하는 것이 보통이지만,  
Java에서는 이렇게 매개 변수의 차이에 따라 하나의 함수를 다양하게 활용할 수 있기 때문에 '과적한다'는 의미의 오버로딩이라는 용어가 붙게 되었습니다.
